...
**Conditional extend blocks.** *Specified by RFC-0036.* An aspect implementation for a
...
> **Implemented** — see `internal/rfcs/4-implemented/rfc-0060-aspect-impl-coherence.md`. The orphan rule, concrete-impl overlap detection, blanket-vs-concrete overlap detection, blanket-impl-aware negative-bound discharge, and negative-impl priority over a blanket positive impl are implemented and enforced (issues #238, #243, #264, #244). Auto-impl aspects remain a separate, still-unimplemented mechanism owned by RFC-0096.
...
**Auto-impl aspects.** The auto-impl mechanism itself is specified by RFC-0096, not by
this coherence section. For coherence purposes, an auto-impl is treated as an ordinary
...
impls. `extend<T: Ord> T[]: Ord` and `extend<T: Hash> T[]: Hash` are natural follow-ons,
deferred until RFC-0062 (Ord Comparison Aspect, draft) and a Hash RFC (none yet) exist.
...
**Array auto-impl propagation.** This is no longer part of RFC-0061's scope. Once RFC-0096's auto-impl mechanism exists, `T[]: Send`/`T[]: Sync`/`T[]: Drop` are each auto-derived exactly when `T` itself has the aspect — an array can cross the same boundaries its elements can, and is dropped by dropping each live element (in reverse index order) before reclaiming the backing storage. When `T: !Drop`, only the backing storage is reclaimed and `T[]: !Drop` holds, permitting move-out from scoped regions.
