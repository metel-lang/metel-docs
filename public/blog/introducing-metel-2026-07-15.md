...
There's a deliberate ordering here that's worth being explicit about, because it inverts what you might expect. Allocators are the language's public face — the thing Metel is nominally "about" — but they are the *last* major piece I plan to build, not the first. The reason is that the allocator design, the more I've worked on it, looks less like a primitive and more like a *combination* of simpler things: an `@a T` is close to an owned box carrying a brand that identifies its allocator; passing an allocator handle down a call tree is close to a general "context parameter" feature that has nothing to do with allocation specifically; and checking that a value doesn't outlive its allocator is just the borrow checker doing its job. If that's right, then allocators aren't a foundation at all — they're what you get when brands, borrow-checking, records, and context parameters are all in place and you point them at storage. So they wait for those, on purpose. The finished allocator design becomes less a feature to implement and more an *acceptance test*: can I rebuild `Heap` and `BumpAlloc` out of the primitives, or is there a genuinely allocator-specific residue left over? Either answer teaches me something.

So: **short term**, the dedicated records post, then an RFC, then `ToRecord`/`FromRecord` and partial-consumption borrows actually working in the interpreter. **Medium term**, the borrow checker, linear types, and brands as real, implemented primitives — and a writeup of the context-parameter idea, which currently exists nowhere. **Then, and only then, allocators**, built on top of all of it. **Long term:** a real compiler and soundness arguments for the parts of Metel that matter most — an interpreter can pressure-test syntax, but it can't carry that weight.
