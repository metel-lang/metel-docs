...
`Int` and `Float` are ergonomic aliases for `i64` and `f64`. Metel also provides exact-width numeric types — `i8`, `i16`, `i32`, `u8`, `u16`, `u32`, `u64`, `f32` — for cases where the bit width matters. Unsuffixed literals like `42` or `3.14` automatically adopt the type the annotation demands:
...
Anything inside `${…}` must be a type that has a `.to_string()` method — all built-in types (`Int`, `Float`, `boolean`, `String`) satisfy this. You can also use full expressions:
