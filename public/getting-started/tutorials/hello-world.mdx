...
`let` bindings are immutable by default. To rebind a variable, use `mut`:
...
`let` and `mut` are not types — they describe whether the binding can be reassigned. Compound assignment operators work on `mut` variables too:
