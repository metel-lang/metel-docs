...
> **Status — under review.** No dependencies on other under-review RFCs; this RFC
...
Two impls of the same aspect for the same type constructor anywhere in the program
are a compile-time error (`T0015 — conflicting implementation`), reported with both
impl spans.

The overlap key for a first cut is the type constructor (e.g. `List`, ignoring type
arguments). This means `impl Aspect for List<i64>` and `impl Aspect for List<String>`
in the same module are a conflict. Non-overlapping parameterised impls are deferred;
see §8.

With the orphan rule in force, overlap can only arise within a single module or
between a module and `std::core`, making detection local and cheap.
...
A marker aspect (an aspect with no methods) may declare an auto-impl rule. The
compiler automatically derives the aspect for any type that satisfies the rule's
field conditions, without requiring an explicit impl.

Auto-impl rules are declared within the aspect definition using a `derive` clause:

```metel
aspect Send {
    derive if { all fields: Send }
}
```

The `derive if` clause reads: "automatically implement this aspect for any struct or
enum all of whose field types implement this aspect." The rule is applied structurally:
for a struct, every field type must satisfy the aspect; for an enum, every field type
in every variant must satisfy the aspect.

The following stdlib aspects use auto-impl rules (RFC-0080):
...
1. **Parameterised overlap.** Whether `impl Aspect for List<i64>` and `impl Aspect
   for List<String>` in the same module should be permitted (non-overlapping) or
   rejected (conflict at constructor level) is deferred. The first cut keys overlap on
   the type constructor; parameterised non-overlap is a future extension.

2. **Coherence across packages.** Coherence is scoped to the single program's module
   graph. A future package system will need a coherence model for separately compiled
   packages; this RFC does not address that.

3. **Specialisation.** Whether a more-specific impl may override a general one is
   deferred. The current model has no specialisation.
