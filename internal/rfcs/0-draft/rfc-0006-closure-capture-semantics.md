...
| RFC-0002 (Aspect Bounds) | `send fun` as a generic type parameter bound (`<F: send fun(T) -> R>`) requires RFC-0002's bound syntax to support the `send` qualifier on function types. Not blocking for capture semantics themselves, but must be resolved before the generics implementation. |
...
5. **`send` annotation at module/library boundaries**
   ...
   **Option B — Unannotated defaults to `fun` (non-Send) at signature boundaries (recommended):** `send`-ness is only propagated inside function bodies. In any exported function signature, an unannotated `fun(T) -> R` is non-`Send`. Authors who want to promise spawnability must write `send fun(T) -> R` explicitly. The contract is then visible and stable:
   ...
   This is consistent with Rust's rule that `pub fn` signatures cannot rely on inference for types that appear in public position, and with Metel's "no implicit conversions" principle — `Send`-ness at API boundaries should be a deliberate declaration, not a side-effect of implementation details.
...
- RFC-0002: `docs/internal/rfcs/rfc-0002-aspect-bound-syntax.md` — aspect bounds on `fun` types (open question 1 and 4)
