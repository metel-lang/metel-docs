...
Every known limitation of the implementation (`LIMIT-*`, on the Architecture Spec)
and every gap in the language specification (`GAP-*`, on the Language Spec) in one
list, including resolved and superseded records. The list is built from the
records when the site is built; nothing here is written by hand.
