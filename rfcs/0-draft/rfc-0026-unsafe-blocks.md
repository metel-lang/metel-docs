...
Concrete cases that safe Metel cannot express:
...
An `unsafe` block is an expression that opts its body out of a specific set of static checks. Outside those checks, the body is otherwise normal Metel code — types are still checked, syntax is still enforced.
...
| Check | Safe Metel | Inside `unsafe` |
|---|---|---|
...
`unsafe` does not propagate. A safe function called from within an `unsafe` block is still subject to all safe-Metel rules. Only the direct body of the `unsafe { }` block or `unsafe fun` is relaxed.
...
Restrict the escape hatch to `extern` declarations that call into C. Unsafe is only required at the FFI boundary; all Metel-to-Metel code is safe.

**Partial merit:** this is simpler and more restricted. However, it does not cover the region allocation (Option B), lock-free data structure, or custom allocator use cases. Those require unsafe operations entirely within Metel code.
...
- RFC-0001: `docs/public/rfcs/rfc-0001-pointer-syntax.md` — pointer arithmetic gated on unsafe; `*T` to linear values requires unsafe
- RFC-0003: `docs/public/rfcs/rfc-0003-concurrency-model.md` — `unsafe_send` bypasses `Send` constraint
- RFC-0006: `docs/public/rfcs/rfc-0006-closure-capture-semantics.md` — unsafe closures; `unsafe fun` as closure literal
- RFC-0024: `docs/public/rfcs/rfc-0024-linear-types.md` — linearity checker relaxed inside unsafe
- RFC-0025: `docs/public/rfcs/rfc-0025-region-allocation.md` — Option B (direct region allocation) requires unsafe
