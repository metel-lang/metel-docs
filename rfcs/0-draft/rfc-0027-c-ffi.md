...
Add a C foreign function interface (FFI) that lets Metel programs call functions exported via the C ABI. Metel declares external functions with `extern "C"` blocks; calling them requires an `unsafe` block. The build tool provides first-class support for Rust crates as the primary use case — any Rust crate can be exposed to Metel by writing a thin `#[no_mangle] extern "C"` shim. Plain C libraries are supported identically. A future RFC will cover auto-binding generation for safe Rust APIs.

---

## Motivation

Metel targets programmers who already think in Rust idioms. A recurring objection to adopting any new language is "but I'd have to rewrite all my libraries." Rust's ecosystem (`crates.io`, ~160k crates as of 2026) is one of the strongest in systems programming. Without a path to reuse it, Metel forces users to choose between the language they want and the libraries they need.
...
Python's trajectory is instructive: the ability to call into C extensions (NumPy, SciPy, PIL) transformed Python from a scripting curiosity into serious infrastructure. Metel's Rust-syntax users have an even stronger pull toward the Rust ecosystem than Python users had toward C.
...
```metel
extern "C" {
    fn metel_json_serialize(ptr: OpaquePtr, len: Int) -> MetelStr;
    fn metel_json_free(ptr: OpaquePtr);
    fn metel_crypto_sha256(input: OpaquePtr, len: Int, out: OpaquePtr);
}
```
...
Extern function calls may violate Metel's memory and type safety invariants — the callee is not checked by the Metel type system. Calls are therefore only permitted inside `unsafe` blocks (see RFC-0026):

```metel
fn serialize(value: SomeType) -> String {
    unsafe {
        let raw = value.as_opaque_ptr();
        let result = metel_json_serialize(raw, value.byte_len());
        result.into_string()
    }
}
```
...
#### 3.1 `MetelStr` and ownership

`MetelStr` is a non-owning, non-null-terminated byte slice. When a Rust binding returns a `MetelStr`, the binding is responsible for lifetime management — Metel does not free it. The binding shim is expected to either:
...
A new `[rust-bindings]` section in `metel.toml` specifies Rust crate dependencies:

```toml
[rust-bindings]
my_metel_bindings = { path = "./bindings/my_metel_bindings" }
```

Each entry is a path to a Rust crate that exposes a `extern "C"` surface. The Metel build tool:
...
3. Links it into the Metel program (static for the compiler, dynamic for the interpreter via `dlopen`).
...
```rust
// bindings/my_metel_bindings/src/lib.rs
...
#[no_mangle]
pub extern "C" fn metel_serialize(input: *const u8, len: usize, out_len: *mut usize) -> *const u8 {
...
#[no_mangle]
pub extern "C" fn metel_serialize_free(ptr: *const u8, len: usize) {
    unsafe {
...
The binding shim author is responsible for memory safety across the boundary. The Metel side wraps the calls in `unsafe` and manages lifetimes through Metel's linear type system (once RFC-0024 is available) or manually.
...
The Metel build tool will emit a warning (not an error) if it detects that a binding shim crate was compiled without `-C panic=abort` and does not use `catch_unwind` at its public surface. Full static enforcement is not feasible at this stage.

---

## Alternatives Considered

### A. Build a Metel standard library from scratch

The most isolation-preserving option. Every library is written in Metel, with no external dependencies. **Rejected** as a sole strategy because it is multi-year work and abandons the Rust ecosystem entirely. A standard library is desirable and should be pursued in parallel (see RFC-0016), but it cannot substitute for FFI in the near term.

### B. WASM as an interop layer

Compile Rust crates to WASM modules; host them in the Metel runtime. **Rejected** because: WASM introduces serialization overhead on every call (memory copy in/out of the WASM heap), prevents sharing memory with the host, and is incompatible with the zero-overhead story in compiler mode. It also makes the interpreter significantly more complex for no semantic benefit over C FFI.

### C. Direct Rust ABI interop (no shim required)

Parse Rust MIR or the crate's public API, auto-generate Metel bindings, and call using Rust's internal ABI. **Rejected for this RFC** because Rust's internal ABI is not stable and changes between compiler versions. Feasible as a future tool (metel-bindgen) that generates C shims automatically and emits both the Rust-side `extern "C"` wrappers and the Metel-side `extern "C"` declarations. This is tracked as a follow-on.
...
1. **`MetelStr` ownership story**: the current proposal punts on owned string returns. Is a `MetelOwnedStr` with an explicit free-function callback the right model before linear types are available, or should this wait for RFC-0024?

2. **Struct passing across FFI**: `#[repr(C)]` Rust structs can be passed by value if both sides agree on the layout. Should Metel support declaring `extern struct` types with explicit field layouts? This would allow richer APIs without opaque-pointer indirection.

3. **Build tool scope**: should `metel.toml` support `crates.io` coordinates directly (triggering a `cargo fetch`), or keep the scope at local paths only? Local paths require the user to vendor dependencies; remote coordinates create a Cargo wrapper.

4. **Auto-binding generation (metel-bindgen)**: a tool that reads a Rust crate's public API and generates both the shim `lib.rs` and the Metel `extern "C"` block would dramatically reduce friction. Should this be scoped into this RFC or tracked separately?

5. **`async` Rust**: `tokio`-based crates are among the most valuable in the ecosystem. Calling into async Rust from synchronous Metel requires a `tokio::Runtime::block_on` wrapper in the shim. Should the build tool or a standard binding helper emit this boilerplate, or is it always the shim author's responsibility?
...
2. **Generics (v0.2)** are complete — the `OpaquePtr` and `MetelStr` types need to fit cleanly into the type system, and the design is cleaner with generics available.
...
- RFC-0024: `docs/public/rfcs/rfc-0024-linear-types.md`
- RFC-0025: `docs/public/rfcs/rfc-0025-region-allocation.md`
- RFC-0026: `docs/public/rfcs/rfc-0026-unsafe-blocks.md`
- RFC-0016: `docs/public/rfcs/rfc-0016-standard-library.md`
