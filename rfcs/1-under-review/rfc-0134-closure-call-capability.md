...
own stdlib signature can simply be written `f: many (T) -> U` and skip inference for
...
So: **fields on `Type::Fun`, no distinct closure type.** The count is two, not one —
§1's storage gap (see there) establishes that `Copy`-ness of a closure is equally
uncomputable from the type unless the type carries it:

```
Type::Fun(Vec<Type>, Box<Type>, call_multiplicity, use_multiplicity)
```
