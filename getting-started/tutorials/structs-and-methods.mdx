...
conceptually spent, and the new binding is the only valid way to reach the value:
...
A type can opt out of this by declaring `Copy`, which makes assignment duplicate the value
...
fun main() {
    let p = Point { x = 1.0, y = 2.0 };
    let q = p;   // copies — both remain usable
    println("${p.x}, ${q.x}");
}
```
...
The compiler tracks moves, but does not reject use of a moved-from binding unless you pass
`--move-check` on the command line. Without it, a program like the first example above would
still let you read `p` after moving it into `q`, silently. Passing `--move-check` turns that
...
- Assignment moves by default; `Copy` opts a type out of that. `Drop` marks a type as needing
  cleanup, though running that cleanup isn't implemented yet. `--move-check` opts into
