...
fun main() {
    var total := 0;
    var cell: &var i64 := &var total;

    var add_one := [&var cell] var || {
        *cell += 1;
    };

    var add_ten := [&var cell] var || {
        *cell += 10;
    };
...
- Writing through a `&var T` requires the explicit dereference operator: `*r = value`.
- Reading a plain value out of a reference is implicit and type-directed; no operator needed.
