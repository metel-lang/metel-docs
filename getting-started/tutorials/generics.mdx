...
fun first<T>(items: T[]) -> Perhaps<T> {
    ...
    return Some { value = items[0u64] };
}

fun main() {
    let names := ["Ada", "Alan", "Grace"];
    match (first(names)) {
        Some { value } => println(value),   // Ada
        None => println("empty"),
    }
...
extend Stack<T> {
    fun peek(self) -> Perhaps<T> {
        ...
        return Some { value = self.items[(self.items.len() - 1) as u64] };
    }
}
...
fun main() {
    ...
    let first: i64 := p.first;
    let second: String := p.second;
    println("${first} / ${second}");   // 1 / one
}
```

Interpolating a generic field directly (`"${p.first}"`) does not yet work — copy it into a locally-typed binding first, as above.
