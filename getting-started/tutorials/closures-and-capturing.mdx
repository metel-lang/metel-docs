...
fun main() {
    let suffix := "!";
    let excite := [suffix.clone()] once |word: String| -> String {
        return word + suffix;
    };
...
fun main() {
    ...
    var matched := 0;
    var matched_ref: &var i64 := &var matched;

    var is_even := [&var matched_ref] var |x: i64| -> boolean {
        if (x % 2 == 0) {
            *matched_ref += 1;
            return true;
        }
