...
**Rationale:** Aspect bounds (#279) and object-safe dispatch (RFC-0008) are future work. Today, every other aspect allows at most one impl per type. Restricting the structured key to `From` keeps the common case simple and the special case explicit.
...
- A future sprint must replace this with a proper `HashMap<(aspect_name, Vec<Type>, target_type), HashMap<method_name, Value>>` impl registry. Tracked as #430.
...
- #416 — Aspect system implementation
- #294 — From aspect and `as` cast upgrade
- #430 — Tech debt: replace flat string-keyed impl env with proper impl registry
