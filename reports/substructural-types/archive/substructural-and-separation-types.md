---
id: substructural-and-separation-types
title: "Substructural and Separation Types in Metel"
type: report
status: archived
created_date: '2026-06-17'
---

> **Archived 2026-07-06.** This report was deleted outright on 2026-06-28 when 11
> memory-model exploration documents were consolidated into a single narrative
> report — an early instance of the "archive vs. delete" inconsistency that
> `reports/substructural-types/README.md` now establishes a policy against. Restored
> from git history (commit `8c1df06`) because its content turned out to still be
> load-bearing: the linear-capability-token pattern here (§6) was independently
> re-derived, three weeks later and without this citation, in
> `reports/substructural-types/linear-types.md`; the typestate pattern (§4) is a
> direct alternative to the row-conditional-impl typestate in
> `reports/substructural-types/structural-records.md` §5; and the Capture Separation
> Calculus (§7–8) is now the working draft behind
> `reports/substructural-types/structured-concurrency.md`.
>
> **Read this as historical record, not as current design.** The syntax throughout
> (`*T`/`*mut T` pointers, `own T` as a unique heap pointer, no allocator/lifetime
> split) predates both RFC-0067 (Reference Types, which replaced `*T`/`*mut T` with
> `&T`/`&mut T`) and the allocator/lifetime split model
> (`reports/memory-model/lifetimes-vs-regions-2026-07-02.md`). `own T` in particular
> reads today as "a uniquely-owned heap pointer" — which is what `@Heap T` already
> is; treat `own T` below as superseded by that, not as an open proposal.
>
