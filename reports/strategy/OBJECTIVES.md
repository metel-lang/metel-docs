...
**Needs a decision from the operator, not just the agent:** RFC-0122 is well behind the
pace its own 07-24 bar ("reach `2-accepted` in v0.12.0") implied — no substantive
change since that day — but **the bar has not actually been missed**: `v0.12.0` has no
tag cut yet, so it's still reachable before release if worked on. See Trigger 27. Does
this get prioritized before the tag cuts, or does the operator want to explicitly defer
it to v0.13.0 now rather than let it stay an ambiguous risk?
...
| 2 | Ownership enforcement and the borrow checker | RFC-0071 **`3-integrated`**, `impl_status: in-progress`; RFC-0122 `0-draft`, behind the pace of its own 07-24 `2-accepted` bar but not yet missed (`v0.12.0` unshipped) | **v0.12.0: #290/#291 shipped + 19 hardening fixes; v0.13.0: #292/#293 + 6 more queued** |
