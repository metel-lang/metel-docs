...
it the brand dependency *that came from fiat-linearity*. **Corrected later the same day:**
this was first written as "the cluster's only dependency on RFC-0076" — an overstatement.
It holds for the by-value conversion pair; RFC-0119's by-reference mode may reinstate one
by a different route (reassembly provenance, RFC-0119 OQ8), unexamined when the split was
made. RFC-0116 is unaffected either way.
